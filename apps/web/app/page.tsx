...
export default function Home() {
  ...
  const { getStatus: getAimmStatusOverride } = useMarketsStatus();

  const { data, loading, error } = useGetMarketsQuery();

  const marketsFromIndexer: Market[] = useMemo(() => data?.markets?.items?.map(mapIndexerMarketToMarket) ?? [], [data]);
  const hasIndexerMarkets = marketsFromIndexer.length > 0;
  const shouldShowOnboarding = isDemoOnboardingMode || (!loading && !error && marketsFromIndexer.length === 0);
  ...
  const filteredMarketsCount = useMemo(() => {
    if (!hasIndexerMarkets) {
      return 0;
    }

    const withResolvedAimmStatus = marketsFromIndexer.map(market => {
    ...
    const filtered = withResolvedAimmStatus.filter(entry => {
      const matchesPlatform = platformFilter === 'all' || entry.market.platform === platformFilter;
      const matchesStatus = statusFilter === 'all' || entry.aimmStatus === statusFilter;

      return matchesPlatform && matchesStatus;
    });

    return filtered.length;
  }, [getAimmStatusOverride, hasIndexerMarkets, marketsFromIndexer, platformFilter, statusFilter]);
            ...
            <CardFooter className='border-border text-muted-foreground bg-muted/20 flex items-center justify-between border-t p-3 text-xs'>
              <span>
                {loading && !error
                  ...
                  : filteredMarketsCount === marketsFromIndexer.length
                    ? `Showing ${marketsFromIndexer.length} indexed markets`
                    : `Showing ${filteredMarketsCount} of ${marketsFromIndexer.length} indexed markets based on your filters`}
              </span>
