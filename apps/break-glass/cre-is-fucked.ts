...
async function main() {
  ...
  try {
    ...
    if (marketIds.length === 0) {
      console.log(
        'No markets found in contract. The contract may be newly deployed or no markets have been onboarded yet.'
      );
      return;
    }
  } catch (error) {
  ...
  for (const marketId of marketIds) {
    try {
      ...
      if (newStatus === MarketStatus.ClosedExternal && marketData.status === MarketStatus.Active) {
        ...
        console.log(
          `Current external prices: A=${marketData.optionACurrentExternalPrice}, B=${marketData.optionBCurrentExternalPrice}`
        );
        console.log(`New prices from Kalshi: A=${optionAPrice}, B=${optionBPrice}`);

        // Only attempt write if prices have actually changed
        if (
          marketData.optionACurrentExternalPrice !== optionAPrice ||
