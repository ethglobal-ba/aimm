...
var configSchema = exports_external2.object({
  ...
  aimmIndexerUrl: exports_external2.string()
});
...
var updateAIMMPrices = (runtime2, workflowResult) => {
  ...
  const resp = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.aimmContractAddress,
...
var updateAllMarkets = (runtime2) => {
  ...
  runtime2.log(`Writing ${marketUpdates.length} market updates to AIMM contract`);
  for (const marketUpdate of marketUpdates) {
    try {
      ...
      runtime2.log(`Error updating AIMM contract for market ${marketUpdate.externalMarketId}: ${error}`);
      continue;
    }
  }
